...
if __name__ == '__main__':
    ...
    if args.generate_clips is True:
        ...
        if n_current_samples <= 0.95*config["n_samples"]:
            logging.info(f"Начинаю генерацию негативных тренировочных аудио для {len(config['target_phrase'])} фраз")
            total_samples_per_phrase = (config["n_samples"] - n_current_samples) // len(config["target_phrase"])
            for idx, target_phrase_single in tqdm(enumerate(config["target_phrase"]), total=len(config["target_phrase"]), desc="Генерация негативных тренировочных аудио"):
                logging.info(f"Генерация адверсариальных текстов и аудио для фразы '{target_phrase_single}' ({idx+1}/{len(config['target_phrase'])})")
                adversarial_texts = config["custom_negative_phrases"].copy()
                adversarial_texts.extend(generate_adversarial_texts(
                    input_text=target_phrase_single,
                    N=total_samples_per_phrase,
                    include_partial_phrase=1.0,
                    include_input_words=0.2))
                generate_samples_onnx(text=adversarial_texts, max_samples=total_samples_per_phrase,
        ...
        if n_current_samples <= 0.95*config["n_samples_val"]:
            logging.info(f"Начинаю генерацию негативных тестовых аудио для {len(config['target_phrase'])} фраз")
            total_samples_per_phrase = (config["n_samples_val"] - n_current_samples) // len(config["target_phrase"])
            for idx, target_phrase_single in tqdm(enumerate(config["target_phrase"]), total=len(config["target_phrase"]), desc="Генерация негативных тестовых аудио"):
                logging.info(f"Генерация адверсариальных текстов и тестового аудио для фразы '{target_phrase_single}' ({idx+1}/{len(config['target_phrase'])})")
                adversarial_texts = config["custom_negative_phrases"].copy()
                adversarial_texts.extend(generate_adversarial_texts(
                    input_text=target_phrase_single,
                    N=total_samples_per_phrase,
